...
# - aqprice_no_pre_no_queue            : select-first (stale Q) + PFE 포인트 게이트, c 없음
# - aqprice_no_pre          : select-first (stale Q + c·queue) + PFE 포인트 게이트
# - aqprice_no_queue        : echo-first (fresh t) + PFE 포인트 게이트, c 없음
# - aqprice      : echo-first (fresh t + c·queue) + PFE 포인트 게이트 (★ 전부 적용)
# - aqprice_no_L : aqprice 와 동일하되 L=0
#
# === Full Echo family (게이트 없음) ===
# - aqprice_no_point            : 매 라우팅 무조건 full echo, fresh t + c·queue 로 선택 (echo 비용 상한선 기준)
ALGORITHMS = [
    # 'q_routing',
    # 'aqfe',
    'aqrerm',
...
COLORS = {
    ...
    'aqprice_no_L': "#3700FF",  # 회색 (L=0, no Route Memory)
}
